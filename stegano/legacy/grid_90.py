...
"""
Grille 90×90 — Structure QR à trois niveaux (LEGACY, hors production)
La Livrée d'Hermès — Anibal Edelberto Amiot (2026)

RETIRÉ DU CHEMIN DE PRODUCTION (décision de l'auteur, 2026-09-12). Ce
module n'est plus maintenu : son rôle d'origine — servir de banc d'essai
pour l'implantation des deux référents et produire du bruit structurel
dans le code — est désormais couvert par les blocs pure/structured de
Carter (stegano/carter.py), eux-mêmes vérifiés par des preuves formelles ;
il n'a donc plus de raison d'être maintenu séparément. Conséquences :

  - Plus exporté par stegano_lib.py (il ne l'a d'ailleurs jamais été via
    stegano_lib -- ses appelants l'importaient directement).
  - Aucun appel depuis le site ou le worker (vérifié 2026-09-12 -- il n'a
    jamais existé qu'en Python pur, jamais porté en JavaScript).
  - Reste sur l'ancien schéma de référent (blue/orange, form_id+color+
    orientation D4), jamais migré vers la règle de lecture v3.
  - Garde sa propre copie minimale de l'ancien chargeur de référent
    (_load_ref256_legacy ci-dessous), car stegano_classic.load_referents()
    -- dont ce module était le dernier appelant réel -- a été supprimé le
    même jour. C'est le même principe déjà appliqué à disk_lib.py, qui
    maintient sa propre copie indépendante pour un usage sans rapport
    (S-box de chiffrement disque).

Conservé ici tel quel, à titre de référence historique, avec ses tests
(voir stegano/legacy/test_grid_90.py, déplacés en même temps).
...
from stegano_classic import zigzag_blocks, _find_ref


def _load_ref256_legacy() -> List:
    """Copie autonome de l'ancien stegano_classic.load_referents() (supprimé
    le 2026-09-12 -- ce module en était le dernier appelant réel). Ancien
    schéma uniquement : data/referent_256.json, liste de dicts blue/orange."""
    with open(_find_ref('referent_256.json')) as f:
        return json.load(f)
