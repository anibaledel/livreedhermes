...
- **Fait, câblage Carter-256** (2026-09-21, première variante migrée — décision explicite de ne pas migrer les sept variantes en un seul passage, le risque le plus élevé du dépôt touchant à du chiffrement en production et une migration de coffre) : `carter.py::encode_carter/decode_carter/carter_capacity` appellent `encrypt_cascade`/`decrypt_cascade` (`_find_grammar_with_c_pub` prend un `capacity_fn`, `max_message_for_cascade` pour Carter-256, `max_message_for` inchangé pour les six autres variantes à l'époque). `js/carter-core.js` de même. Les 9 vecteurs `carter256-*` de `vectors/carter_v3.json` régénérés sous cascade (k1/k2/alg capturés). `C_PUB['carter256']` recalibré (`tools/recalibrate_carter_v3.py --variants carter256 --n-keys 10000`, désormais paramétrable par variante) : **399 → 399, inchangé** — la marge existante (capacité moyenne ~507 octets pour une cible de 399) absorbe les 29 octets de surcoût sans relever le taux de redraw au-delà du seuil de 1 % (mesuré à 0,64 % pour C_PUB=399). Coût de la campagne : ~106 minutes pour N=10 000 (recherche par dichotomie, chaque point de mesure ré-exécutant la recherche de grammaire complète sur les 10 000 clés). 145/145 tests Python, 39/39 tests JS verts, aucune régression sur les six autres variantes (capacity_fn inchangé pour elles).
- **Fait, câblage Carter-360 et Carter-Mix, Python seulement** (2026-09-21) : `carter.py::encode_carter_360/decode_carter_360/carter360_capacity` et `encode_carter_mix/decode_carter_mix/carter_mix_capacity` appellent `encrypt_cascade`/`decrypt_cascade`, même schéma que Carter-256 (`_nonce1`/`_nonce2`, `capacity_fn=max_message_for_cascade`). **Pas de port JS** : `js/carter-core.js` n'a jamais eu la grammaire/positions de ces deux variantes (contrairement à Carter-Random, déjà porté) — les câbler en JS aurait mélangé deux chantiers à risques distincts (le port de géométrie et le changement de chiffrement) sur les mêmes vecteurs, décision explicite de les traiter séparément. Aucune régression : ces variantes n'existaient pas en JS, aucune interopérabilité perdue. Port JS suivi par [l'issue #85](https://github.com/anibaledel/livreedhermes/issues/85). Vecteurs `carter360-basic-01`/`cartermix-basic-01` régénérés sous cascade (k1/k2/alg capturés, `nonce1`/`nonce2`).
- **Recalibration C_PUB, Carter-360/Mix** (`tools/recalibrate_carter_v3.py --variants carter360,cartermix --n-keys 10000 --apply`) : **contrairement à Carter-256, les deux ont bougé** — `carter360` : 1861 → 1848 (-13), `cartermix` : 1872 → 1852 (-20), tous deux mesurés < 1 % de redraw (0,83 % et 0,67 % respectivement) avec 0 % d'échec. La généralisation attendue depuis le rapport de câblage Carter-256 (marge relative plus large pour ces deux variantes, donc surcoût plus facilement absorbé) ne s'est **pas** vérifiée telle quelle : la marge est bien plus large en valeur absolue (mean_cap ≈ 2370 pour une cible ≈ 1850, contre ≈ 507 pour une cible 399 sur Carter-256), mais la courbe redraw-vs-C_PUB est nettement plus raide ici (22 % de redraw dès +370 au-dessus du C_PUB retenu, contre une marge bien plus tolérante sur Carter-256) — la recalibration a donc réellement changé la valeur retenue, même si l'écart final reste faible (moins de 1 % de la capacité moyenne). **Réponse à la question posée** : le recalibrage doit être refait variante par variante, le résultat de Carter-256 ne se généralise pas automatiquement aux autres — seule la méthode (dichotomie sur le taux de redraw réel, mesuré contre les fonctions de production) se transporte.
- **Pas encore fait** : câblage des quatre variantes restantes (random/18/hybrid/classic) ; câblage des pages web au-delà de ce qu'`encodeur.html` obtient déjà gratuitement (aucun appelant public ne passe de nonce, donc aucun changement d'API côté pages) ; migration `secubox/vault_lib.py` (tag `3`) — deux exigences posées avant d'y toucher : un coffre au format existant doit rester lisible, et l'octet d'algorithme doit distinguer les deux formats sans ambiguïté à la lecture ; port JS de Carter-360/Mix (issue #85).
